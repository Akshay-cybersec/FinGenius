...
const InvestmentSimulator = () => {
  ...
  const renderLearningOverlay = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 animate-in fade-in">
        <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-700">
          ...
          <div className="p-8 min-h-[300px] flex flex-col justify-center">
            {isQuiz ? (
              <div className="space-y-6 animate-in slide-in-from-right duration-300">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">🧠 Quick Quiz</h3>
                <p className="text-lg text-slate-600 dark:text-slate-300">{currentCard.question}</p>
                <div className="space-y-3">
                  {currentCard.options?.map((opt, idx) => (
                    <button
                      ...
                      className="w-full p-4 text-left rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-500 transition-all font-medium"
                    >
                      {opt}
                    </button>
                  ))}
              ...
              <div className="space-y-6 animate-in slide-in-from-right duration-300">
                <h3 className="text-2xl font-extrabold text-slate-800 dark:text-white">{currentCard.title}</h3>
                <p className="text-lg text-slate-600 dark:text-slate-300 leading-relaxed">{currentCard.content}</p>
                ...
                {currentCard.analogy && (
                  <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border-l-4 border-indigo-500">
                    ...
                    <p className="text-slate-700 dark:text-slate-200 italic">"{currentCard.analogy}"</p>
                  </div>
                )}
                ...
                <div className="pt-6 flex justify-end">
                  <button
                    onClick={handleNext}
                    className="flex items-center gap-2 px-6 py-3 bg-slate-900 dark:bg-white dark:text-slate-900 text-white rounded-full font-bold hover:scale-105 transition-transform"
                  >
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-950 flex justify-center gap-2">
  ...
  return (
    <div className="flex h-full w-full bg-slate-50 dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 overflow-hidden font-sans p-6 gap-6 transition-colors duration-300 relative overflow-x-hidden">
      ...
      {showWelcome && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl max-w-lg w-full text-center border border-slate-200 dark:border-slate-700 relative overflow-hidden">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="w-full py-3 text-slate-500 dark:text-slate-400 font-medium hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors"
              >
                Skip Intro
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsFdModalOpen(false)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-slate-600 dark:text-slate-400">Amount ($)</label>
                <input type="number" value={fdAmount} onChange={e => setFdAmount(Number(e.target.value))} className="w-full p-2 border rounded-lg bg-slate-50 dark:bg-slate-900 dark:border-slate-600" />
              </div>
              <div>
                <label className="text-sm text-slate-600 dark:text-slate-400">Tenure (Years)</label>
                <input type="number" value={fdTenure} onChange={e => setFdTenure(Number(e.target.value))} className="w-full p-2 border rounded-lg bg-slate-50 dark:bg-slate-900 dark:border-slate-600" />
              </div>
      ...
      <div className="flex-1 flex flex-col gap-6 h-full overflow-hidden">
        ...
        <div className="flex flex-col gap-4 flex-shrink-0">
          ...
          <div className="tour-tabs flex gap-2 p-1 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md rounded-xl border border-slate-200 dark:border-slate-700/50 w-fit shadow-sm overflow-x-auto">
        ...
        <div className="tour-quick-trades flex-shrink-0 bg-white/60 dark:bg-slate-800/30 backdrop-blur-sm border border-slate-200 dark:border-slate-700/50 rounded-2xl overflow-hidden shadow-md flex flex-col max-h-60 transition-all">
                        ...
                        <td className="p-3 text-right pr-6">
                          <div className="flex justify-end items-center gap-2">
                            ...
                            <button
                              ...
                              className={`px-3 py-1 text-xs font-bold bg-red-100 hover:bg-red-500 text-red-600 hover:text-white rounded transition-all shadow-sm ${index === 0 ? 'tour-sell-all-btn' : ''}`}
        ...
        <div className="tour-market-table flex-1 bg-white/60 dark:bg-slate-800/30 backdrop-blur-sm border border-slate-200 dark:border-slate-700/50 rounded-2xl overflow-hidden shadow-xl flex flex-col min-h-0 relative">
          {activeTab === 'Portfolio' ? (
                        ...
                        tradingPortfolio.map(item => {
                          ...
                          return (
                            <tr key={item.ticker} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                              ...
                              <td className="p-4 text-right font-mono text-slate-600 dark:text-slate-400">{item.quantity}</td>
                              <td className="p-4 text-right font-mono text-slate-500">${item.buy_price.toFixed(2)}</td>
                              <td className="p-4 text-right font-mono text-slate-700 dark:text-slate-200">${asset.price.toFixed(2)}</td>
                              <td className={`p-4 text-right font-bold font-mono ${isProfit ? 'text-emerald-500' : 'text-red-500'}`}>{isProfit ? '+' : ''}{pl.toFixed(2)}</td>
                            </tr>
                          )
                        })
                      )}
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700/30 text-sm">
                  {filteredAssets.length === 0 ? (
                    <tr><td colSpan={4} className="p-8 text-center text-slate-400 dark:text-slate-500">No assets available in this category.</td></tr>
                  ) : (
                    filteredAssets.map((asset, index) => (
                      <tr key={asset.ticker} className={`group hover:bg-slate-100 dark:hover:bg-slate-700/20 transition-colors ${index === 0 ? 'tour-asset-row-0' : ''}`}>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-right tour-buy-action">
                          <div className="flex justify-end gap-2">
                            {activeTab !== 'Fixed Deposits' && (
                              <input
                                ...
                                className={`w-16 bg-slate-200 dark:bg-slate-700 rounded-lg px-2 text-xs text-center focus:outline-none focus:ring-1 focus:ring-cyan-500 ${index === 0 ? 'tour-stock-input-0' : ''}`}
                              />
                            )}
                            <button
      ...
      <div className="w-80 flex flex-col gap-6 flex-shrink-0">
        ...
        <div className="tour-insights flex-1 bg-white/60 dark:bg-slate-800/20 rounded-2xl border border-slate-200 dark:border-slate-700/30 p-5 flex flex-col gap-4 shadow-lg dark:shadow-none">
