      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-6xl mx-auto space-y-10 p-6 relative">
          ...
          {showXpInfo && (
            ...
            >
              ...
              <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
                <div className="flex justify-between"><span>Correct Answer</span><span className="font-bold text-green-500">+50 XP</span></div>
                <div className="flex justify-between"><span>Speed Bonus</span><span className="font-bold text-blue-500">Up to +30 XP</span></div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">
                  <p className="font-bold mb-1">Next Level ({userProgress.level + 1})</p>
                  <div className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden mb-1">
                    <div className="h-full bg-primary" style={{ width: `${progressPercent}%` }} />
                  </div>
                  <p className="text-xs text-slate-400">{userProgress.xp} / {nextLevelThreshold} XP</p>
                </div>
              </div>
            </motion.div>
          )}
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <motion.div variants={itemVariants}>
            <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 tracking-tight mb-2">
              Quiz Arena
            </h1>
            <p className="text-slate-400 text-lg font-medium">Prove your financial literacy.</p>
          </motion.div>
          
          <motion.div variants={itemVariants} className="flex gap-4">
             {/* Level Badge */}
             <div className="flex items-center gap-3 bg-slate-800/50 backdrop-blur-sm px-5 py-3 rounded-2xl border border-slate-700 relative overflow-hidden group hover:border-primary/50 transition-colors cursor-pointer" onClick={() => setShowXpInfo(!showXpInfo)}>
                ...
                <div className="p-2 bg-slate-700 rounded-xl text-yellow-400 shadow-inner"><Star size={24} fill="currentColor" /></div>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Level {userProgress.level}</p>
                  <p className="font-bold text-white text-lg">{userProgress.xp} XP</p>
                </div>
                <Info size={16} className="text-slate-500 group-hover:text-primary transition-colors ml-2" />
             </div>
          </motion.div>
        </div>
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((cat) => (
            <motion.div
              ...
              className={`group cursor-pointer bg-slate-900 rounded-[2rem] p-8 border border-slate-800 relative overflow-hidden transition-all duration-300 ${cat.shadow} hover:border-transparent`}
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between">
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 text-xs font-bold text-slate-300 border border-slate-700">
                    <Zap size={14} className="text-yellow-400 fill-yellow-400" /> 10 Questions
                  </span>
                  <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center group-hover:bg-white group-hover:text-slate-900 text-white transition-all duration-300">
        ...
        <motion.div 
          variants={itemVariants} 
          className="relative rounded-[2rem] bg-gradient-to-r from-slate-900 to-slate-950 p-10 text-white overflow-hidden shadow-2xl shadow-indigo-500/20 border border-indigo-500/20 group"
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-8">
              <div className="p-5 bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 shadow-xl">
                 <BrainCircuit size={48} className="text-indigo-400 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 text-xs font-bold uppercase tracking-wider border border-indigo-500/30">Daily Event</span>
                </div>
                <h3 className="text-3xl font-black mb-2 tracking-tight">Rapid Fire Challenge</h3>
                <p className="text-slate-400 max-w-lg font-medium text-lg">Double XP enabled! Test your reflexes with 5 random market scenario questions.</p>
              </div>
            </div>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }} 
              className="bg-white text-slate-900 px-10 py-5 rounded-2xl font-bold shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:shadow-[0_0_50px_rgba(255,255,255,0.5)] transition-all flex items-center gap-3 text-lg"
            >
    ...
    return (
      <div className="min-h-[80vh] flex flex-col max-w-5xl mx-auto py-4 px-4 relative justify-center">
        ...
        <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
            <div className={`absolute top-20 left-10 w-96 h-96 bg-gradient-to-br ${activeCategory.color} opacity-20 blur-[150px] rounded-full animate-pulse`} />
            <div className={`absolute bottom-20 right-10 w-80 h-80 bg-gradient-to-tr ${activeCategory.color} opacity-10 blur-[100px] rounded-full`} />
        </div>
          ...
          {showExitConfirm && (
            ...
            >
              <motion.div 
                ...
                className="bg-slate-900 p-8 rounded-3xl shadow-2xl border border-slate-700 max-w-sm w-full text-center"
              >
                <div className="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6 text-red-500">
                  <AlertTriangle size={32} />
                </div>
                <h3 className="text-2xl font-bold text-white mb-2">Quit Quiz?</h3>
                <p className="text-slate-400 mb-8">You will lose all progress for this session.</p>
                <div className="flex gap-4">
                  ...
                  <button onClick={() => setGameState("menu")} className="flex-1 py-3 rounded-xl font-bold bg-red-600 text-white hover:bg-red-500 transition-colors">Quit</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <div className="relative z-20 flex flex-col gap-6 mb-4">
           ...
           <div className="flex justify-between items-center">
               {/* CATEGORY BADGE */}
               <div className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-xl border border-slate-700/50 text-slate-300">
               ...
               {/* TIMER BADGE */}
               <div className={`flex items-center gap-3 px-5 py-2.5 rounded-xl font-mono font-bold text-xl shadow-lg transition-colors ${timer < 5 ? "bg-red-500/20 text-red-500 border border-red-500/50 animate-pulse" : "bg-slate-800 text-white border border-slate-700"}`}>
                  <Clock size={20} />
                  <span>{timer}s</span>
               </div>

               <button onClick={() => setShowExitConfirm(true)} className="p-3 hover:bg-slate-800 rounded-xl text-slate-400 hover:text-white transition-colors">
                  <X size={24} />
               </button>
           </div>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                       <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold border transition-colors shrink-0 ${showCorrect ? "bg-emerald-500 text-white border-emerald-500" : showWrong ? "bg-red-500 text-white border-red-500" : "border-slate-600 text-slate-500 group-hover:border-white group-hover:text-white"}`}>
                         {String.fromCharCode(65 + idx)}
                       </div>
                       <span className={`text-lg font-semibold transition-colors ${textColor} group-hover:text-white`}>{option}</span>
                    </div>
                    
                    {showCorrect && <Check className="text-emerald-400" size={20} />}
                    {showWrong && <X className="text-red-400" size={20} />}
                  </motion.button>
                );
              })}
            </div>
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center max-w-2xl mx-auto py-12 px-4 text-center relative z-10">
        <motion.div 
          ...
          className="bg-slate-900/80 backdrop-blur-xl p-10 rounded-[3rem] border border-slate-700 shadow-2xl w-full"
        >
          {hasLeveledUp && (
          ...
          <div className="w-28 h-28 mx-auto bg-gradient-to-b from-slate-800 to-slate-900 rounded-full flex items-center justify-center mb-8 border border-slate-700 shadow-inner">
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="p-6 bg-slate-800 rounded-3xl border border-slate-700/50">
              <div className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-2">XP Earned</div>
              <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-500">+{sessionScore}</div>
            </div>
            <div className="p-6 bg-slate-800 rounded-3xl border border-slate-700/50">
               <div className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-2">Accuracy</div>
              <div className="text-4xl font-black text-white">{correctCount} <span className="text-xl text-slate-600 font-medium">/ {activeCategory.questions.length}</span></div>
            </div>
          </div>

          <div className="mb-8 text-left bg-slate-800/50 rounded-2xl p-2 max-h-64 overflow-y-auto custom-scrollbar border border-slate-700/50">
          ...
          <button onClick={() => setGameState("menu")} className="w-full bg-white text-slate-900 hover:bg-slate-200 py-5 rounded-2xl font-bold text-lg shadow-xl shadow-white/10 transition-all transform hover:scale-[1.02]">
