...
export default function QuizFeature() {
  ...
  if (gameState === "menu") {
    ...
    return (
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-6xl mx-auto space-y-10 p-6 relative bg-background text-foreground">
        {/* XP INFO MODAL */}
        <AnimatePresence>
          {showXpInfo && (
            <motion.div 
              ...
              className="absolute top-24 right-6 z-50 w-80 bg-card text-card-foreground rounded-2xl shadow-2xl border border-border p-6"
            >
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-bold text-lg flex items-center gap-2">
                  <Star className="text-yellow-500" size={18} fill="currentColor" /> XP System
                </h3>
                <button onClick={() => setShowXpInfo(false)} className="text-muted-foreground hover:text-foreground"><X size={18} /></button>
              </div>
              ...
              <div className="space-y-4 text-sm text-muted-foreground">
                ...
                <div className="bg-muted p-3 rounded-lg">
                  <p className="font-bold mb-1 text-foreground">Next Level ({userProgress.level + 1})</p>
                  <div className="w-full h-2 bg-secondary rounded-full overflow-hidden mb-1">
                    <div className="h-full bg-primary" style={{ width: `${progressPercent}%` }} />
                  </div>
                  <p className="text-xs text-muted-foreground">{userProgress.xp} / {nextLevelThreshold} XP</p>
                </div>
              </div>
            </motion.div>
          )}
  ...
  <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-foreground via-foreground to-foreground/40 tracking-tight mb-2">
    Quiz Arena
  </h1>
  <p className="text-muted-foreground text-lg font-medium">
    Prove your financial literacy.
  </p>
</motion.div>
          <motion.div variants={itemVariants} className="flex gap-4">
             <div className="flex items-center gap-3 bg-card/50 backdrop-blur-sm px-5 py-3 rounded-2xl border border-border relative overflow-hidden group hover:border-primary/50 transition-colors cursor-pointer" onClick={() => setShowXpInfo(!showXpInfo)}>
                ...
                <div className="p-2 bg-muted rounded-xl text-yellow-500 dark:text-yellow-400 shadow-inner"><Star size={24} fill="currentColor" /></div>
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Level {userProgress.level}</p>
                  <p className="font-bold text-foreground text-lg">{userProgress.xp} XP</p>
                </div>
                <Info size={16} className="text-muted-foreground group-hover:text-primary transition-colors ml-2" />
             </div>
          </motion.div>
        </div>
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((cat) => (
            <motion.div
              ...
              className={`group cursor-pointer bg-card text-card-foreground rounded-[2rem] p-8 border border-border relative overflow-hidden transition-all duration-300 ${cat.shadow} hover:border-transparent`}
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-muted-foreground text-sm mb-8 leading-relaxed font-medium">{cat.description}</p>
                
                <div className="flex items-center justify-between">
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted text-xs font-bold text-muted-foreground border border-border">
                    <Zap size={14} className="text-yellow-500 fill-yellow-500" /> 10 Questions
                  </span>
                  <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center group-hover:bg-foreground group-hover:text-background transition-all duration-300">
        ...
        <motion.div 
          variants={itemVariants} 
          className="relative rounded-[2rem] bg-gradient-to-r from-indigo-600 to-slate-800 p-10 text-white overflow-hidden shadow-2xl shadow-indigo-500/20 border border-indigo-500/20 group"
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-8">
              <div className="p-5 bg-white/10 backdrop-blur-md rounded-3xl border border-white/10 shadow-xl">
                 <BrainCircuit size={48} className="text-indigo-300 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-3 py-1 rounded-full bg-indigo-500/30 text-indigo-100 text-xs font-bold uppercase tracking-wider border border-indigo-400/30">Daily Event</span>
                </div>
                <h3 className="text-3xl font-black mb-2 tracking-tight">Rapid Fire Challenge</h3>
                <p className="text-indigo-100 max-w-lg font-medium text-lg">Double XP enabled! Test your reflexes with 5 random market scenario questions.</p>
              </div>
            </div>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }} 
              className="bg-white text-indigo-900 px-10 py-5 rounded-2xl font-bold shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:shadow-[0_0_50px_rgba(255,255,255,0.5)] transition-all flex items-center gap-3 text-lg"
            >
  ...
  if (gameState === "playing") {
    ...
    return (
      <div className="min-h-[80vh] flex flex-col max-w-5xl mx-auto py-4 px-4 relative justify-center bg-background text-foreground">
        <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
            <div className={`absolute top-20 left-10 w-96 h-96 bg-gradient-to-br ${activeCategory.color} opacity-10 blur-[150px] rounded-full animate-pulse`} />
            <div className={`absolute bottom-20 right-10 w-80 h-80 bg-gradient-to-tr ${activeCategory.color} opacity-5 blur-[100px] rounded-full`} />
        </div>
        ...
        <AnimatePresence>
          {showExitConfirm && (
            <motion.div 
              ...
              className="absolute inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm rounded-3xl"
            >
              <motion.div 
                initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
                className="bg-card text-card-foreground p-8 rounded-3xl shadow-2xl border border-border max-w-sm w-full text-center"
              >
                ...
                <h3 className="text-2xl font-bold mb-2">Quit Quiz?</h3>
                <p className="text-muted-foreground mb-8">You will lose all progress for this session.</p>
                <div className="flex gap-4">
                  <button onClick={() => setShowExitConfirm(false)} className="flex-1 py-3 rounded-xl font-bold bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors">Cancel</button>
                  <button onClick={() => setGameState("menu")} className="flex-1 py-3 rounded-xl font-bold bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors">Quit</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <div className="relative z-20 flex flex-col gap-6 mb-4">
           ...
           <div className="flex justify-between items-center">
               <div className="flex items-center gap-3 bg-card/80 backdrop-blur-md px-4 py-2 rounded-xl border border-border text-card-foreground">
               ...
               <div className={`flex items-center gap-3 px-5 py-2.5 rounded-xl font-mono font-bold text-xl shadow-lg transition-colors ${timer < 5 ? "bg-red-100 dark:bg-red-500/20 text-red-500 border border-red-500/50 animate-pulse" : "bg-card text-card-foreground border border-border"}`}>
                  <Clock size={20} />
                  <span>{timer}s</span>
               </div>

               <button onClick={() => setShowExitConfirm(true)} className="p-3 hover:bg-muted rounded-xl text-muted-foreground hover:text-foreground transition-colors">
                  <X size={24} />
               </button>
           </div>
        </div>
          ...
          >
            <div className="mb-8 px-4 text-center">
               <motion.h2 
                 ...
                 className="text-2xl md:text-4xl font-black leading-tight drop-shadow-sm"
               >
                 {question.q}
               </motion.h2>
            </div>
              ...
              {question.options.map((option: string, idx: number) => {
                ...
                return (
                  <motion.button
                    ...
                    whileHover={!isAnswered ? { scale: 1.02, backgroundColor: "var(--muted)" } : {}}
  ...
  if (gameState === "result") {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center max-w-2xl mx-auto py-12 px-4 text-center relative z-10 bg-background text-foreground">
        <motion.div 
          initial={{ scale: 0.9, opacity: 0 }} 
          animate={{ scale: 1, opacity: 1 }} 
          className="bg-card text-card-foreground backdrop-blur-xl p-10 rounded-[3rem] border border-border shadow-2xl w-full"
        >
          {hasLeveledUp && (
          ...
          <div className="w-28 h-28 mx-auto bg-muted rounded-full flex items-center justify-center mb-8 border border-border shadow-inner">
            <Trophy size={56} className="text-yellow-500 drop-shadow-md" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="p-6 bg-muted rounded-3xl border border-border">
              <div className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-2">XP Earned</div>
              <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">+{sessionScore}</div>
            </div>
            <div className="p-6 bg-muted rounded-3xl border border-border">
                <div className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-2">Accuracy</div>
              <div className="text-4xl font-black">{correctCount} <span className="text-xl text-muted-foreground font-medium">/ {activeCategory.questions.length}</span></div>
            </div>
          </div>

          <div className="mb-8 text-left bg-muted/50 rounded-2xl p-2 max-h-64 overflow-y-auto border border-border">
                     ...
                     <div key={i} className="p-4 border-b border-border last:border-0">
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 ...
                                 <p className="text-xs text-muted-foreground">Correct: {item.options[item.correct]}</p>
                             </div>
                         </div>
                     </div>
                 );
          ...
          <button onClick={() => setGameState("menu")} className="w-full bg-foreground text-background hover:opacity-90 py-5 rounded-2xl font-bold text-lg shadow-xl transition-all transform hover:scale-[1.02]">
